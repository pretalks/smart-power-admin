      ...
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {subsidyStates.map((state, i) => (
            <motion.div key={state.slug} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}>
              <Link to={`/govt-subsidy/${state.slug}`} className="block bg-background rounded-2xl overflow-hidden shadow-card hover:shadow-elevated transition-all group">
                ...
                <div className="p-6">
                  ...
                  <p className="text-sm text-muted-foreground line-clamp-2">{state.subsidy.slice(0, 100)}...</p>
